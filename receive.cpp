...
const std::string CLIENT_ID("kaylor_office_PC_rev");
const std::string REV_TOPIC("kaylor/action");
const std::string SEND_TOPIC("kaylor/feedback");
const int QOS = 0;
...
class callback : public virtual mqtt::callback,
  ...
  void connected(const std::string &cause) override {
    ...
    std::cout << "\nSubscribing to topic '" << REV_TOPIC << "'\n"
              << "\tfor client " << CLIENT_ID
              << " using QoS" << QOS << "\n"
              << "\nPress Q<Enter> to quit\n" << std::endl;

    cli_.subscribe(REV_TOPIC, QOS, nullptr, subListener_);
  }
